...
export const PreviewRefresh = () => {
  ...
  return (
    <div className={`fixed bottom-8 right-8 z-[100] transition-all duration-500 transform ${isPending ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0 pointer-events-none'}`}>
      <div className="bg-businessx-black/90 backdrop-blur-md text-businessx-yellow border border-businessx-yellow/30 px-4 py-2 rounded-xl shadow-2xl flex items-center gap-3">
        <div className="relative flex h-3 w-3">
          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-businessx-yellow opacity-75"></span>
          <span className="relative inline-flex rounded-full h-3 w-3 bg-businessx-yellow"></span>
        </div>
