...
const QuoteCard = ({ quote }: QuoteCardProps) => {
  return (
    <div className="bg-terminal-black border border-terminal-border p-4 hover:border-businessx-orange transition-all group flex flex-col gap-4" data-testid="quote-card">
      <div className="flex justify-between items-start">
        <div>
          <div className="flex items-baseline gap-3 mb-1">
            <span className="text-xl font-black font-display uppercase text-terminal-white group-hover:text-businessx-orange transition-colors">
      ...
      <LocalizedClientLink href={`/account/quotes/${quote.id}`} className="w-full mt-auto">
        <Button data-testid="quote-details-link" className="w-full bg-transparent border border-terminal-border hover:border-businessx-orange text-terminal-dim hover:text-businessx-orange font-bold uppercase tracking-widest text-xs h-10 rounded-none shadow-none transition-all">
          View Quote
        </Button>
      </LocalizedClientLink>
    </div>
